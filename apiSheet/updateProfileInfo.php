...
if(
    ...
    if ($result == 1) { 
        ...
        $query = "SELECT u.id as user_id, u.f_name, u.profile_pic, u.bio, u.l_name, u.gender, u.username, u.is_dpt_head, u.can_approve, u.email_notice, u.email, u.phone, u.country, u.city, u.postal_addr AS address, u.reset as token, c.id AS department_id, c.desc department, d.id AS role_id, d.desc as role, e_desc.desc AS alt_desc from users u LEFT JOIN code_desc c ON u.dept = c.id LEFT JOIN code_desc e_desc ON u.email_notice = e_desc.id left JOIN code_desc d ON u.role = d.id WHERE u.id = '$user_id'";
        $result = mysqli_query($conn, $query);
