...
function get_status_count($status_id, $department_id, $conn){
    ...
    if($status_id == 116 ){
        ...
        $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE (CURRENT_DATE > end_date AND p.is_approved=1 AND p.is_archive =0) AND (SELECT AVG(t.completion)<100 from tasks t WHERE t.project_id = p.project_id) AND p.dept_id = '$department_id'";
    }
    elseif($status_id == 85 ){
        // $query = "SELECT COUNT(project_id) total_overdue FROM projects WHERE NOW()>end_date AND dept_id = '$department_id'";
    ...
    elseif($status_id == 126 ){
        // $query = "SELECT COUNT(project_id) total_overdue FROM projects WHERE NOW()>end_date AND dept_id = '$department_id'";
        $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE ( p.priority=73) AND p.dept_id = '$department_id' AND p.is_archive =0";
    }
    // elseif($status_id == 88){
