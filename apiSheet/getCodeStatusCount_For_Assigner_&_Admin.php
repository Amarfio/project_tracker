...
function get_total_task_count( $conn){
    ...
    $query = "SELECT COUNT(t.status) total_task_count FROM tasks t LEFT JOIN projects p ON p.project_id = t.project_id LEFT JOIN users u ON u.id =  t.assigned_to ";
...
function get_approved_count($conn){
    ...
    $query = "SELECT COUNT(t.task_id) approved_project FROM tasks t LEFT JOIN projects p ON p.project_id = t.project_id LEFT JOIN users u ON u.id =  t.assigned_to WHERE p.is_approved = 1  ";
...
function get_unapproved_count($conn){
    ...
    $query = "SELECT COUNT(t.task_id) unapproved_project FROM tasks t LEFT JOIN projects p ON p.project_id = t.project_id LEFT JOIN users u ON u.id =  t.assigned_to WHERE p.is_approved = 0 ";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if (!isset($codes[$row['init']])) {
                ...
                $codes[$row['init']] = array(
                    ...
                    "name" => $row['name'],
                    "code_desc" => array(),
                    "total_tasks" => get_total_task_count( $conn ),
