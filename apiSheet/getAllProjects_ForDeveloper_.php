...
function query_project_by_status($status_id, $user_id)
{
    ...
    // $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department,cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE pro.status = '$status_id' AND (pro.owner = '$user_id' OR pro.s_owner = '$user_id') AND pro.is_archive = 0 ORDER BY pro.project_id DESC";
    $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department,cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE pro.status = '$status_id' AND (pro.posted_by = '$user_id') AND pro.is_archive = 0 ORDER BY pro.project_id DESC";
    ...
    if ($status_id == 116) {
        ...
        // $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, 'overdue' status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (CURRENT_DATE > end_date AND pro.is_approved=1) AND (SELECT AVG(t.completion)<100 AND pro.is_archive = 0 from tasks t WHERE t.project_id = pro.project_id) AND (pro.owner = '$user_id' OR pro.s_owner = '$user_id') ORDER BY pro.project_id DESC";
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, 'overdue' status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (CURRENT_DATE > end_date AND pro.is_approved=1) AND (SELECT AVG(t.completion)<100 AND t.status <> 58 AND pro.is_archive = 0 from tasks t WHERE t.project_id = pro.project_id) AND (pro.posted_by = '$user_id') ORDER BY pro.project_id DESC";
    } elseif ($status_id == 85) {
        // $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (pro.is_approved=1) AND (pro.owner = '$user_id' OR pro.s_owner = '$user_id') AND pro.is_archive = 0 ORDER BY pro.project_id DESC";
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (pro.is_approved=1) AND (pro.posted_by = '$user_id') AND pro.is_archive = 0 ORDER BY pro.project_id DESC";
    } elseif ($status_id == 131) {
        // $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (pro.owner = '$user_id' OR pro.s_owner = '$user_id') AND pro.is_archive = 1 ORDER BY pro.project_id DESC";
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (pro.posted_by = '$user_id' OR pro.s_owner = '$user_id') AND pro.is_archive = 1 ORDER BY pro.project_id DESC";
    }
    //   elseif($status_id == 88) {
