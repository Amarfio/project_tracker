...
require_once 'functions/getProjectCondition.php';
...
function query_total_project($conn){ 
    ...
    $query_total_project = "SELECT * FROM vw_last_update_projects";
  ...
  function query_project_by_status ($status_id, $owner, $start_date, $end_date){
    ...
    //   $query_project_by_status = "SELECT pro.*, co.id version_id, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE pro.status = '$status_id' ORDER BY pro.project_id DESC";
      ...
      if($status_id == 120){
        ...
        if($owner == "" || $owner == "undefined"){
            $query_project_by_status = "SELECT * FROM vw_last_update_projects_app WHERE last_update_date >='$start_date' AND last_update_date <= '$end_date' ORDER BY last_update_date DESC ";
        }else{

            $query_project_by_status = "SELECT * FROM vw_last_update_projects_app WHERE last_update_date >='$start_date' AND last_update_date <= '$end_date' AND owner_id = '$owner' ORDER BY last_update_date DESC ";
        }
        // echo($query_project_by_status); die();
    }
    //   elseif($status_id == 88){
    ...
    if ($num > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $projects_tasks[] =  array(
                ...
                "last_update" => $row['last_update'],
                "last_update_date" => $row['last_update_date'],
                "condition"=> getProjectCondition($row['start_date'], $row['end_date'], $row["completion"])
            );
        }
...
if ( isset($_GET['status_id'])) {
     ...
     $owner_id = mysqli_escape_string($conn, $_GET['user_id']);
     $start_date = mysqli_escape_string($conn, $_GET['start_date']);
     $end_date = mysqli_escape_string($conn, $_GET['end_date']);
    //  echo ("here the owner dey ".$owner_id); die();
    // echo '<br>'. $user_id; 
    // echo '<br>'. $status_id; 
     result_from_query ($conn, query_project_by_status ($status_id, $owner_id, $start_date, $end_date));
