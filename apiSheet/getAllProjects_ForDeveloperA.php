...
function noOfOverdueTasks($project_id, $conn){
    $query = "SELECT COUNT(task_id) totalTasks FROM tasks where project_id = '$project_id' AND NOW() > tasks.end_date";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_array($result);
...
function query_total_project($conn, $user_id){ 
    ...
    if ($num > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            //code to get the number of overdue tasks for a project
            $noOfOverdueTasks = noOfOverdueTasks($row['project_id'], $conn);
            ...
            $projects_tasks[] =  array(
                ...
                "no_of_overdue_tasks"=> $noOfOverdueTasks,
  ...
  function query_project_by_status ($status_id, $user_id){
    ...
    //   $query_project_by_status = "SELECT pro.*, co.id version_id, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE pro.status = '$status_id' AND pro.dept_id = '$user_id' ORDER BY pro.project_id DESC";
      ...
      //check if the status is priority the codesc value for priority is 126
        ...
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE pro.owner = '$user_id' AND pro.is_archive = 1 ORDER BY pro.project_id DESC";
      }
    //   elseif($status_id == 88) {
  ...
  function query_by_status_and_date($user_id, $status_id, $start_date, $end_date, $client_id){
    ...
    if($user_id == 'undefined' && $status_id =='undefined'){
        ...
        $query_by_status_and_date = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department,cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE pro.is_archive = 0 AND pro.owner= '$user_id' AND pro.client= '$client_id' AND (pro.start_date >= '$start_date' AND pro.end_date <= '$end_date') ORDER BY pro.project_id DESC";
    }
    else if($user_id == 'undefined'){
        $query_by_status_and_date = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department,cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE pro.is_archive = 0 AND pro.status= '$status_id' AND pro.client= '$client_id' AND (pro.start_date >= '$start_date' AND pro.end_date <= '$end_date') ORDER BY pro.project_id DESC";
    }
    else if($client_id == 'undefined'){
        $query_by_status_and_date = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department,cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE pro.is_archive = 0 AND pro.status= '$status_id' AND pro.owner= '$user_id' AND (pro.start_date >= '$start_date' AND pro.end_date <= '$end_date') ORDER BY pro.project_id DESC";
    }
    else {
        ...
        //code to check for approved projects...
            ...
            $query_by_status_and_date = "SELECT pro.*, co.id version_id, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, 'overdue' status_desc, co_dept.id department_id, co_dept.desc department,cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE (CURRENT_DATE > end_date AND pro.is_approved=1) AND (SELECT AVG(t.completion)<100 AND pro.is_archive = 0 from tasks t WHERE t.project_id = pro.project_id) AND pro.owner = '$user_id' AND pro.status='$status_id' AND pro.client= '$client_id' AND (pro.start_date >= '$start_date' AND pro.end_date <= '$end_date') ORDER BY pro.project_id DESC";
        }
    }
...
if ( isset($_GET['status_id']) && isset($_GET['user_id'])) {
    ...
    // die();
     ...
     if(isset($_GET['start_date']) && isset($_GET['end_date'])){
        ...
        // die();
        result_from_query($conn, query_by_status_and_date($user_id, $status_id, $start_date, $end_date, $client_id));
     }
     else{
