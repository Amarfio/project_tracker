...
function get_total_project_count( $conn, $client_id){
    ...
    $query = "SELECT COUNT(p.project_id) total_project_count FROM projects p WHERE p.is_archive = 0 AND p.client = '$client_id'";
...
function get_status_count($status_id, $client_id, $conn){
    ...
    $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE  p.status  = '$status_id' AND p.is_archive = 0 AND p.client= '$client_id'";
   
    

    if($status_id == 116){
        $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE (CURRENT_DATE > end_date AND p.is_approved=1) AND p.is_archive = 0 AND p.client= '$client_id' AND (SELECT AVG(t.completion)<100 from tasks t WHERE t.project_id = p.project_id) ";
    }
    elseif($status_id == 85){
        $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE (p.is_approved=1) AND p.is_archive = 0 AND p.client= '$client_id'";
    }
    // elseif($status_id == 86){
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if (!isset($codes[$row['init']])) {
                ...
                $codes[$row['init']] = array(
                    "init" => $row['init'],
                    "code_desc" => array(),
                    "total_projects" => get_total_project_count( $conn, $client_id )
                );
            }
