sheetApp.controller(
  ...
  function (
    ...
    $scope.updateUserInfo = function (emailNotice, dept) {
      ...
      var userDepartment = parseInt($("#department_id").val());

      //get the selected value for emailNotice
      if (email_notice == "" || email_notice == undefined) {
        var email_notice = $scope.user.emailNotice;
        console.log(email_notice);
        // return false;
      }

      //default the value to is active
      if (isWorking == "" || isWorking == undefined) {
        var isWorking = $scope.user.is_active;
      ...
      $http({
        ...
        function successCallback(response) {
          ...
          if (res.status == "success") {
            $timeout((window.location = "users"), 5000);
