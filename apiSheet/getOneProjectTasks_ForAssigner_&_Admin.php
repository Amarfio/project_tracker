...
require_once 'functions/noOfConflictingTasks.php';
    ...
    function get_all_tasks ($project_id, $status_id, $conn){
        $task_arr = array();
        ...
        $query = "select * from vw_tasks_under_project_by_id where project_id = '$project_id' AND status_id = '$status_id' ORDER BY task_id DESC";
...
if (isset($_GET['project_id'])) {
    ...
    $t_status_id = mysqli_escape_string($conn, $_GET['status_id']);
    ...
    if ($num > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $projects_tasks[] =  array(
                ...
                "tasks" => get_all_tasks($row['project_id'], $t_status_id, $conn)
            );
        }
