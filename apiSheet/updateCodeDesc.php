...
if (isset($data) && isset($data->user_id) && isset($data->code_id) && isset($data->description) ) {
    // echo json_encode($data);

    $user_id = mysqli_real_escape_string($conn, $data->user_id);
    $id = mysqli_real_escape_string($conn, $data->code_id);
    ...
    // $query = "INSERT INTO `code_desc` (`init`, `init_desc`, `desc`) VALUES ('$init', '$init_desc', '$description')";
    $query = "UPDATE `code_desc` SET `desc` = '$description' WHERE `code_desc`.`id` = '$id'";
    ...
    if ($result == 1) {
        ...
        $message = json_encode(
            array(
                'message' => $description . '( ' . $description . ' )' . ' updated successfully',
                'status' => 'success'
                // 'data' => [
                //     'init' => $init,
                //     'init_desc' => $init_desc,
            ...
            $user =  $user_id;
            $activity =  ' tried to add code->' . $description . ' and code_desc->'.$description. '| Details: ' . $message;
            $status = 'success';
            log_activity($conn, $user, $activity, $status, getSecurity());
