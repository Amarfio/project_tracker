...
if(
    //stuff to add later start and end date, status
    isset($data) && isset($data->name) && isset($data->owner) 
){
    ...
    $postedby = mysqli_real_escape_string($conn, $data->postedby);
    ...
    if(isDescriptionAvailable($conn, "programs", $description) === false){
        $query = "INSERT INTO `programs` (`id`, `name`, `description`, `owner`, `created_at`, `updated_at`, `file_name`, `posted_by`) 
                  VALUES 
                  (NULL, '$name', '$description', '$owner', NOW(), NOW(), '$fileName', '$postedby')";
        // echo ($query);
        // die();
        $result = mysqli_query($conn, $query);
        ...
        if ($result == 1) {
            ...
            $message = json_encode(
                ...
                $user =  $postedby;
                $activity =  'tried to create a program PROG-0000' . $program_id . ' | Details: ' . $message;
                $status = 'success';
            ...
            // }
                ...
                $user =  $postedby;
                $activity =  ' tried to create a program PROG-0000 ';
                $status = 'failed';
                log_activity($conn, $user, $activity, $status, getSecurity());
