...
function get_status_count($status_id, $conn){
    ...
    //overdue projects
    if($status_id == 116){
        $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE (CURRENT_DATE > end_date AND p.is_approved=1) AND p.is_archive = 0 AND (SELECT AVG(t.completion)<100 from tasks t WHERE (t.status = 59 OR t.status = 61) AND t.project_id = p.project_id) ";
    }
    elseif($status_id == 85){
    ...
    elseif($status_id == 131){
        $query = "SELECT COUNT(p.status) total_project_count FROM projects p WHERE p.is_archive = 1";
    }
    // elseif($status_id == 86){
