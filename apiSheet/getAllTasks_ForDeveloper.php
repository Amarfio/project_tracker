...
if (isset($_GET['user_id']) && !isset($_GET['status_id']) && !isset($_GET['is_approved'])) {
    $user_id = mysqli_escape_string($conn, $_GET['user_id']); 
    // echo '<br>'. $user_id; 
    // echo '<br>'. $status_id; 

    result_from_query ($conn, query_total_tasks ($user_id));
}
elseif (isset($_GET['user_id']) && isset($_GET['status_id'])) {
    $user_id = mysqli_escape_string($conn, $_GET['user_id']);
    $status_id = mysqli_escape_string($conn, $_GET['status_id']);
    // echo '<br>'. $user_id; 
    // echo '<br>'. $status_id; 

    result_from_query ($conn, query_task_by_status ($user_id, $status_id));
}
elseif (isset($_GET['user_id']) && isset($_GET['is_approved'])) {
    $user_id = mysqli_escape_string($conn, $_GET['user_id']);
    $is_approved = mysqli_escape_string($conn, $_GET['is_approved']);
    // echo '<br>'. $user_id; 
    // echo '<br>'. $status_id; 

    result_from_query ($conn, query_task_by_is_approved ($user_id, $is_approved));
}
