...
function get_total_project_count( $conn, $user_id){
    ...
    $query = "SELECT COUNT(pro.project_id) total_project_count FROM projects pro WHERE (pro.owner = '$user_id' OR pro.s_owner = '$user_id') AND pro.is_archive = 0";
...
function get_status_count($status_id, $user_id, $conn){
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if (!isset($codes[$row['init']])) {
                ...
                $codes[$row['init']] = array(
                    "init" => $row['init'],
                    "code_desc" => array(),
                    "total_projects" => get_total_project_count( $conn, $user_id)
                );
            }
            ...
            $codes[$row['init']]['code_desc'][] = array(
                ...
                "status_count" =>get_status_count($row['id'], $user_id, $conn)
            );
        }
