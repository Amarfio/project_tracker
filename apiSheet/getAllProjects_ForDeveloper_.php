...
function query_total_project($conn, $user_id)
{
    ...
    $query_total_project = "SELECT pro.*, co.id version_id, cod_pri.desc, CONCAT(proj_owner.f_name,' ', proj_owner.l_name) as m_o, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc cod_pri ON cod_pri.id = pro.priority LEFT JOIN users proj_owner ON proj_owner.id = pro.owner WHERE  (pro.owner = '$user_id' OR pro.s_owner = '$user_id' OR pro.posted_by = '$user_id') AND pro.is_archive = 0 ORDER BY pro.project_id DESC";
