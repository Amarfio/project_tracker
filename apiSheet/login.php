...
if (isset($data)) {
    ...
    if (!isset($data->username) || !isset($data->password)) {
        ...
        if ($num > 0) {
            ...
            // LOG ACTIVITY 
            $lastResetDate = date($row['p_reset_at']);
            $todaysDate = date("Y-m-d");
            $numOfDays = date_diff(date_create($lastResetDate), date_create($todaysDate));
            $days = $numOfDays->format("%a");

            // echo($lastResetDate." ". $todaysDate."dates ".$days);
            // die();

            if($row['p_reset_at']== NULL) {
                // echo("here we dey"); die();

                echo json_encode(expiredPasswordResponse());
            }
            elseif($days>=90){
                echo json_encode(expiredPasswordResponse());
            }
...
function expiredPasswordResponse(){
    return array(
        'message'=> 'Password has expired, kindly reset it!',
