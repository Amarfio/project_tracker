...
function get_total_task_count( $conn, $department_id, $is_dept_head, $user_id, $project_id){
    ...
    $query = "SELECT COUNT(t.status) total_task_count FROM tasks t LEFT JOIN projects p ON p.project_id = t.project_id LEFT JOIN users u ON u.id =  t.assigned_to  WHERE  t.assigned_to = '$user_id' AND u.is_dpt_head = '$is_dept_head'  AND t.project_id = '$project_id' ";
...
function get_status_count($status_id, $conn, $department_id, $is_dept_head, $user_id, $project_id){
...
function get_approved_count($conn, $department_id, $is_dept_head, $user_id, $project_id){
    ...
    $query = "SELECT COUNT(t.task_id) approved_project FROM tasks t LEFT JOIN projects p ON p.project_id = t.project_id LEFT JOIN users u ON u.id =  t.assigned_to WHERE p.is_approved = 1 AND t.assigned_to = '$user_id' AND u.is_dpt_head = '$is_dept_head'  AND t.project_id = '$project_id' ";
...
function get_unapproved_count($conn, $department_id, $is_dept_head, $user_id, $project_id){
    ...
    $query = "SELECT COUNT(t.task_id) unapproved_project FROM tasks t LEFT JOIN projects p ON p.project_id = t.project_id LEFT JOIN users u ON u.id =  t.assigned_to WHERE p.is_approved = 0 AND t.assigned_to = '$user_id' AND u.is_dpt_head = '$is_dept_head' AND t.project_id = '$project_id' ";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if (!isset($codes[$row['init']])) {
                ...
                $codes[$row['init']] = array(
                    ...
                    "name" => $row['name'],
                    "code_desc" => array(),
                    "total_tasks" => get_total_task_count( $conn, $department_id, $is_dept_head, $user_id, $project_id),
                    "approved_tasks" => get_approved_count($conn, $department_id, $is_dept_head, $user_id, $project_id),
                    "unapproved_tasks" => get_unapproved_count($conn, $department_id, $is_dept_head, $user_id, $project_id)
                );
            }
            ...
            $codes[$row['init']]['code_desc'][] = array(
                ...
                "status" => $row['desc'],
                "status_count" =>get_status_count($row['id'], $conn, $department_id, $is_dept_head, $user_id, $project_id)
            );
        }
