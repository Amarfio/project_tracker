    ...
    function(
        ...
        $scope.loadNewClientMembers = function() {
            return $http
                ...
                .catch(function(error) {
                    console.error("Error loading client members from external API:", error);
        ...
        $scope.loadClientMembers = function(clientId) {
            ...
            console.log("Filtered members for client ID", clientId, ":", $scope.members);
        ...
        $scope.showAlert = function(message) {
            var alertContainer = angular.element(document.getElementById("alertContainer"));
        ...
        $scope.addParticipant = function() {
            if (
                ...
                $scope.currentPipeline.participants.indexOf($scope.selectedParticipant) === -1
            ) {
            ...
            $http
                ...
                .then(function(response) {
                    ...
                    if (response.data.success) {
                        ...
                        console.error("Error logging action as discussion:", response.data.message);
                    ...
                    if (response.data.success) {
                        ...
                        $scope.selectedPipeline = {
                            ...
                            nextDate: pipeline.next_date_formatted ?
                                new Date(pipeline.next_date_formatted) : null,
                        ...
                        $scope.loadDiscussions(pipelineId);
                        $scope.loadSourceOwners();
                        console.log("Final currentPipeline object:", $scope.currentPipeline);
        ...
        $scope.loadDiscussions = function(pipelineId) {
            $http
                .get("apiSheet/pipeline.php?action=get_discussions&pipeline_id=" + pipelineId)
                .then(function(response) {
                    if (response.data.success) {
                        $scope.currentPipeline.discussions = response.data.data.map(function(discussion) {
                            return {
                                id: discussion.id,
        ...
        $scope.savePipeline = function() {
            ...
            if (
                ...
                Swal.fire({
                    ...
                    text: "Next date must be after the discussion date (" + formattedDiscussionDate + ")."
                });
                $scope.isSaving = false;
                return;
            }
                    ...
                    if (response.data && response.data.success) {
                        ...
                        $timeout(function() {
                            ...
                            if (isEditingExisting) {
                                ...
                                var newPipelineId = response.data.data && response.data.data.id ?
                                    response.data.data.id : 'Unknown';

                                Swal.fire({
                                    icon: "success",
                                    title: "Pipeline Created!",
                                    text: response.data.message || "Pipeline created successfully.",
                                    confirmButtonColor: "#5e72e4"
                                }).then(function() {
                                    $scope.loadPipelines();
                                    $scope.resetForm();
                        ...
                        Swal.fire({
                            ...
                            text: response.data && response.data.message ?
                                response.data.message : "Unknown server response"
                        });
                    }
                })
                .catch(function(error) {
                            ...
                            .then(function(response) {
                                if (response.data.success) {
                                    ...
                                    Swal.fire({
                                        icon: "error",
                                        title: "Error",
                                        text: response.data.message || "Failed to suspend pipeline."
                                    });
                                }
                            })
                            .catch(function(error) {
