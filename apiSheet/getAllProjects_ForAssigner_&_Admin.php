...
function query_total_project($conn){ 
  ...
  function query_project_by_status ($status_id){
    ...
    //   $query_project_by_status = "SELECT pro.*, co.id version_id, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE pro.status = '$status_id' ORDER BY pro.project_id DESC";
      $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(pro_owner.f_name,' ',pro_owner.l_name) AS project_owner, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN users pro_owner ON pro.owner = pro_owner.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE pro.status = '$status_id' AND pro.is_archive = 0 ORDER BY pro.project_id DESC";
      ...
      if($status_id == 116){
        ...
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(pro_owner.f_name,' ',pro_owner.l_name) AS project_owner, co.desc version, pro.status status_id, 'overdue' status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN users pro_owner ON pro.owner = pro_owner.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE (CURRENT_DATE > pro.end_date AND pro.is_approved=1 AND pro.is_archive = 0) AND(SELECT AVG(t.completion)<100 from tasks t WHERE (t.status = 59 OR t.status = 61) AND t.project_id = pro.project_id) ORDER BY pro.project_id DESC";
      }
    //   elseif($status_id == 88){
    ...
    //   }
        ...
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(pro_owner.f_name,' ',pro_owner.l_name) AS project_owner, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN users pro_owner ON pro.owner = pro_owner.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE (pro.is_approved=1 AND pro.is_archive = 0) ORDER BY pro.project_id DESC";
     }
     elseif($status_id ==131){
        //code for scheduled projects thus id 86 in the codesc table 
        $query_project_by_status = "SELECT pro.*, co.id version_id, CONCAT(pro_owner.f_name,' ',pro_owner.l_name) AS project_owner, co.desc version, pro.status status_id, co_p_status.desc status_desc, co_dept.id department_id, co_dept.desc department, cli_p.name client_name, DATEDIFF(NOW(), pro.start_date) AS no_of_days FROM projects pro LEFT JOIN code_desc co ON pro.version_no = co.id LEFT JOIN users pro_owner ON pro.owner = pro_owner.id LEFT JOIN code_desc co_dept ON pro.dept_id = co_dept.id LEFT JOIN clients cli_p ON cli_p.client_id = pro.client LEFT JOIN code_desc co_p_status ON co_p_status.id = pro.status WHERE (pro.is_archive = 1) ORDER BY pro.project_id DESC";
     }
