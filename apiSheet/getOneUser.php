...
if (isset($_GET['user_id'])) {
    ...
    $query = "SELECT u.id as user_id, u.f_name, u.profile_pic, u.signature, u.bio, u.l_name, u.gender, u.username, u.is_dpt_head, u.can_approve, u.email, u.phone, u.country, u.city, u.email_notice, e_desc.desc AS alt_desc, u.postal_addr AS address, u.reset as token, c.id AS department_id, c.desc department, d.id AS role_id, d.desc as role, u.is_active AS is_active from users u LEFT JOIN code_desc c ON u.dept = c.id LEFT JOIN code_desc e_desc ON u.email_notice = e_desc.id left JOIN code_desc d ON u.role = d.id WHERE u.id = '$user_id'";
