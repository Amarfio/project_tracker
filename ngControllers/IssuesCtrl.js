sheetApp.controller('IssuesCtrl', function ($scope, dataService, check_auth, myConfig, $http, $location, $localStorage, $location) {
    ...
    $scope.get_my_issues = function (){
        ...
        var externalApiUrl = 'https://issues.unionsg.com/api/incidents.php';
        if($scope.user_info.role == 'developer' && $scope.user_info.is_dept_head =='1'){
            var externalApiUrl = "https://issues.unionsg.com/api/incidents.php?department_id="+$scope.user_info.deptIssues
        }
        else if($scope.user_info.role == 'developer' && $scope.user_info.is_dept_head =='0') {
            externalApiUrl = 'https://issues.unionsg.com/api/incidents.php?email='+ $scope.user_info.email
        }
